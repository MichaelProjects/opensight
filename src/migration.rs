...
pub fn check_db_tables(connection_str: String) {
    ...
    let queries = ["CREATE TABLE user_access (userid text NOT NULL, username text NOT NULL, email text NOT NULL, password text NOT NULL, creation_date TIMESTAMP WITH TIME ZONE NOT NULL, power_level text NOT NULL, CONSTRAINT PK_user PRIMARY KEY ( userid ));", "CREATE TABLE analytics(tracking_id text NOT NULL, creation_time TIMESTAMP WITH TIME ZONE NOT NULL, os text NOT NULL, device_size text NOT NULL, session_length bigint NOT NULL, session_id text NOT NULL, CONSTRAINT PK_analytics PRIMARY KEY ( tracking_id ));", "CREATE TABLE applications ( application_id text NOT NULL, application_name text NOT NULL, created_time timestamp NOT NULL, token text NOT NULL, os text NOT NULL, CONSTRAINT PK_applications PRIMARY KEY ( application_id ) );"];
    let mut counter = 0;
